...
the `## BLUF` task-doc convention in the shared `CLAUDE.md` ("Task documents").
...
cold-execution default (see "Task documents" in `CLAUDE.md`): a fresh reader (a new LLM session, you
months later, a colleague) should grasp the whole point of the task from the BLUF alone, then use
`## Context` to orient and the body to act — with none of the conversation that produced the task.
