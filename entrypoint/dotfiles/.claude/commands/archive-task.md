...
10. **Triage the task's ad-hoc scripts — promote or remove** (per the "Ad-hoc scripts" convention in `~/.claude/CLAUDE.md`). If `tasks/adhoc/$ARGUMENTS/` exists, go through each script and decide:
    ...
    After triaging, drop the now-empty `tasks/adhoc/$ARGUMENTS/` dir. If the dir didn't exist, skip silently. Report what you promoted (with any gate-wiring proposals) and what you removed.
