...
10. **Triage the task's ad-hoc scripts — promote or remove** (per the "Ad-hoc scripts" convention in `~/.claude/CLAUDE.md`). If `tasks/adhoc/$ARGUMENTS/` exists, go through each script and decide:
    ...
    After triaging every script you know about, the `tasks/adhoc/$ARGUMENTS/` dir **should be empty** — drop it. Treat that emptiness as a **spot-check**: **if anything is left over, it's a file you didn't account for — flag it to me, don't silently delete it.** If the dir didn't exist, skip silently. Report what you promoted (with any gate-wiring proposals) and what you removed.
