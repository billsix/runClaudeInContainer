---
description: Abandon a stack entry we are NOT going to do (not the same as completing it)
argument-hint: <stack number, or empty for the top>
---

Drop a stack entry we have decided **not** to do. This is deliberately separate from
`/stack-pop`, which means "finished".

The stack file is `~/.claude/stack.md`.

Steps:

1. Identify the entry: `$ARGUMENTS` is its number from `/stack`; empty means the top.
